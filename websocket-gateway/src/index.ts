...
io.on('connection', (socket: AuthenticatedSocket) => {
  ...
  socket.on('group:invite', async ({ group_id, invited_user_ids, group_name }) => {
    ...
    invited_user_ids.forEach((userId: string) => {
  ...
  socket.on('friend:request_action', async ({ type, to_user_id }) => {
    ...
    let eventType: 'received' | 'accepted' | 'declined' | 'cancelled'
    switch (type) {
      case 'sent':
        eventType = 'received'
        break
  ...
  // Handle reaction add
  socket.on('reaction:add', async ({ message_id, conversation_id, conversation_type, emoji }) => {
    console.log(`[Reaction] ${socket.username} reacting ${emoji} to ${message_id} in ${conversation_type}:${conversation_id}`)
    ...
    try {
      ...
      // Broadcast to everyone in room (including sender for cross-tab sync)
      ...
      // Also broadcast to user's personal room for other tabs not in this room
      io.to(`user:${socket.userId}`).emit('reaction:updated', {
        message_id,
        conversation_id,
        conversation_type,
        emoji,
      ...
      console.log(`[Reaction] Added ${emoji} to ${message_id}`)
    } catch (error) {
  ...
  socket.on('reaction:remove', async ({ message_id, conversation_id, conversation_type, emoji }) => {
    console.log(`[Reaction] ${socket.username} removing ${emoji} from ${message_id} in ${conversation_type}:${conversation_id}`)
    ...
    try {
      ...
      console.log(`[Reaction] Removed ${emoji} from ${message_id}`)
    } catch (error) {
...
function stopTyping(socket: AuthenticatedSocket, room: string) {
...
async function broadcastSidebarUpdate(
  conversation_id: string,
  conversation_type: string,
  last_message: string,
